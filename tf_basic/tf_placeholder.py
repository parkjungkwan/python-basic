...
'''
    ...
    상수 Constant
    변수 Variable
    플레이스홀더 Placeholder  이 있다.
    플레이스홀더는 placeholder 연산 노드를 가리키는 텐서(다차원배열)이며,
    텐서플로우에서 그래프를 실행할 때 사용자가 데이터를 주입할 수 있는 통로

    tf.placeholder 와 tf.Variable 의 차이점
    tf.Variable 을 사용하면 선언 할 때 초기값을 반드시 제공해야 한다.
    tf.placeholder 를 사용하면 초기값을 제공할 필요가 없으므로 Session.run 의
    feed_dict 인수를 사용하여 런타임을 지정할 수 있다.
'''
